<template>
  <div class="main-cnt" :style="backgroundDiv">
    <div class="music-btn" @click="dialogVisible=true">申请音乐人认证</div>
    <el-dialog title="申请音乐人认证" :visible.sync="dialogVisible" center width="40%" append-to-body>
      <div class="d-cnt">
        <div class="d-line">
...
export default {
  data() {
    return {
      dialogVisible: false,
      backgroundDiv: {
        backgroundImage: 'url(' + require('../assets/img/music.jpg') + ')'
      },
      form: {
        id: '',
        name: '',
  ...
  methods: {
    clickApply() {
      this.axios.post('/api/musician', this.form).then(e => {
        let res = e.data
        if (res.success === 1) {
          this.$message.success('认证成功')
          this.$store.commit('setUserType', 'musician')
        } else {
